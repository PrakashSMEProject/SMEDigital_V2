 ...
 * </ol>
 * Get an instance of this class by making a call to <code>BeanMapperFactory.getMapperInstance( CoverDetsVOToTTrnPremiumVOHolderReverse.class )</code>.
 */
public class CoverDetsVOToTTrnPremiumVOHolderReverse extends BaseBeanToBeanMapper<com.rsaame.pas.vo.svc.TTrnPremiumVOHolder, com.rsaame.pas.vo.bus.CoverDetailsVO>{
	private final Logger log = Logger.getLogger( this.getClass() );	

	public CoverDetsVOToTTrnPremiumVOHolderReverse(){
		super();
	}

	public CoverDetsVOToTTrnPremiumVOHolderReverse( com.rsaame.pas.vo.svc.TTrnPremiumVOHolder src, com.rsaame.pas.vo.bus.CoverDetailsVO dest ){
		super( src, dest );
	}
	...
	public com.rsaame.pas.vo.bus.CoverDetailsVO mapBean(){
		...
		if(  !Utils.isEmpty( beanA.getPrmPremium() ) ){
			com.rsaame.pas.cmn.converter.BigDecimalDoubleConverter converter = ConverterFactory.getInstance( com.rsaame.pas.cmn.converter.BigDecimalDoubleConverter.class, "", "" );
 			beanB.setPremiumAmt( converter.getBFromA(  beanA.getPrmPremium() ) ); 
 		}
		...
		if(  !Utils.isEmpty( beanA.getPrmPremiumActual() ) ){
			com.rsaame.pas.cmn.converter.BigDecimalDoubleConverter converter = ConverterFactory.getInstance( com.rsaame.pas.cmn.converter.BigDecimalDoubleConverter.class, "", "" );
 			beanB.setPremiumAmtActual( converter.getBFromA(  beanA.getPrmPremiumActual() ) ); 
 		}
		...
		if(  !Utils.isEmpty(beanA.getPrmSumInsured())){
			com.rsaame.pas.cmn.converter.BigDecimalDoubleConverter converter = ConverterFactory.getInstance( com.rsaame.pas.cmn.converter.BigDecimalDoubleConverter.class, "", "" );
 			beanB.getSumInsured().setSumInsured( converter.getBFromA(beanA.getPrmSumInsured()) );  
 		}
		/* Mapping: "prmvalue1" -> "sumInsured.eDesc" */
		if(  !Utils.isEmpty(beanA.getPrmValue1())){
 			beanB.getSumInsured().seteDesc( beanA.getPrmValue1().toString() );  
 		}
		/* Mapping: "ValidityStartDate -> "vsd" */
		if( !Utils.isEmpty( beanA.getPrmValidityStartDate() )){
			beanB.setVsd( beanA.getPrmValidityStartDate() );
		}
