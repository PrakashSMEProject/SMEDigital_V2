...
public class DropDownRendererHepler{
	...
	public static LookUpListVO getLookFilteredList( LookUpListVO lookUpL, HttpSession session ){
		...
		String loggedInUser = "";
		Boolean passUpdatedList = Boolean.FALSE;
		...
		for( LookUpVO lkv : lookupList ){
			if( lkv.getCategory().equalsIgnoreCase( "RSA_USER_PT" ) ){
				passUpdatedList = Boolean.TRUE;
				String userRole = SvcUtils.getLookUpDescription( "USER_ROLE_PAS", "ALL", "ALL", lkv.code.intValue() );
				if( ( ! Integer.valueOf(lkv.code.toString()).equals( userProfile.getRsaUser().getUserId() ) ) 
						&& ( getHighestRoleRank( userRole ) < getHighestRoleRank( loggedInUser ) 
								|| (getHighestRoleRank( loggedInUser )==1 && getHighestRoleRank( userRole ) == getHighestRoleRank( loggedInUser ) ) ) ){
		...
		if(!Utils.isEmpty( lookupList ) && !Utils.isEmpty( lookupList.get( 0 ).getCategory() ) &&  lookupList.get( 0 ).getCategory() .equals( SCHEME_CATEGORY )){
			...
			while(iterator.hasNext()){
				...
				if(!Utils.isEmpty( lkv ) && !Utils.isEmpty( lkv.getDescription() )){
					// Code is added to set Direct scheme as default for Home/Travel
					if (lkv.getCode().toString().equals(Utils.getSingleValueAppConfig( "HOME_DIRECT_SCHEME" )) || lkv.getCode().toString().equals(Utils.getSingleValueAppConfig( "TRAVEL_INSURANCE_SCHEME" ))){
						newLookupList.add( CopyUtils.copySerializableObject( lkv ) );
						iterator.remove();
					}				
					else if(lkv.getDescription().toLowerCase().contains( "flexi" ) && !(lkv.getDescription().toLowerCase().contains( "pre-pack" )) ){
						newLookupList.add( CopyUtils.copySerializableObject( lkv ) );
						iterator.remove();
					}
				}
		...
		if( ( !Utils.isEmpty( newLookupList ) && newLookupList.size() > 0 ) || passUpdatedList ){
