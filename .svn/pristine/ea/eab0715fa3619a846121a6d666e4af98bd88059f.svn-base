...
/**
 * @author m1014644
 *Renders the ACL based on the status of the quote of policy or quote
 ...
 */
public class StatusBasedACL extends BaseVisibilityRule{
	...
	protected VisibilityLevel calculateVisibilityLevel( HttpServletRequest request, Map<String, String> input ){
		...
		VisibilityLevel statusVisibility = getPrivilegeForUserUtil.getVisibility( request, input, AppConstants.POL_QUO_STATUS);	
	...
	protected String getCaseIdentifier( Map<String, String> input ){
		...
		for( Map.Entry<String, String> identifier : input.entrySet() ){
			caseIdentifier = Utils.concat( "S_" + identifier.getKey(), identifier.getValue() );
			break;
		}
