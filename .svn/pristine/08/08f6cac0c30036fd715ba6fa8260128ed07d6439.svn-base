...
public abstract class SBSJob implements Job {
	...
	protected EplatformWsStaging batchResponse(EplatformWsStaging eplatformWsStaging, BaseVO baseVO) {
		...
		try {
			...
			try {
				CreateSBSQuoteResponse createSBSQuoteResponse = (CreateSBSQuoteResponse) WSAppUtils
						.getObjectFromJsonStrin(WSAppUtils.getStringFromClob(eplatformWsStaging.getQuoResponse()),
								CreateSBSQuoteResponse.class);
				createSBSQuoteResponse.setQuoteStatus("" + policyVO.getStatus().byteValue());
				eplatformWsStaging.setQuoResponse(
						Hibernate.createClob(WSAppUtils.getJsonStringFromObjectPrettyPrint(createSBSQuoteResponse)));
	...
	protected JobExecutor createExecutor(String eventName) {
		if (CREATE_QUOTE.equalsIgnoreCase(eventName) || UPDATE_QUOTE.equalsIgnoreCase(eventName)||CONVERT_TO_POLICY.equalsIgnoreCase(eventName)) {
